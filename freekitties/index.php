...
foreach( $xml->children() as $child )
{
	switch( $child->getName() )
	{
		...
		case("projects"):
			...
			foreach( $child->children() as $subchild )
			{
				$projects[$i][$subchild->getName()] = $subchild;
				$i++;
			}
			break;	
		case("awards"):
			...
			<div class="uk-grid">
				<div id="navigation" class="uk-width-medium-1-4">
					...
					<ul class="uk-nav uk-nav-side">
						...
						if( count($features) > 0 ) echo'<li><a href="#features">Features</a></li>';
						...
						echo '<li><a href="#credits">Team / Credits</a></li>
...
if( count($projects) > 0)
{
    echo '<h2 id="projects">Projects o</h2>
		  <ul>';
	...
	if ($handle = opendir('.')) {
		while (false !== ($entry = readdir($handle))) {
			if ($entry != "." && $entry != ".." && substr($entry,0,1) != "_" && strpos($entry, ".") === FALSE && substr($entry,-4) != ".log" && substr($entry,0,6) != "images" && substr($entry,0,8) != "trailers" && substr($entry,0,9) != "error_log") {
				echo '<li><a href="sheet.php?p='.$entry.'">'.ucwords(str_replace("_", " ", $entry)).'</a></li></ul>';
			}
		}
	}
	closedir($handle);
